...
//	uint8_t out[] = {JEDEC_READ_STATUS};  //0xd7
...
int32_t PIOS_Flash_Jedec_PrintID()
{
	...
	PIOS_SPI_TransferByte(flash_dev->spi_id, JEDEC_DEVICE_ID);
	printf(" dev %02x %02x",
			(uint8_t )(PIOS_SPI_TransferByte(flash_dev->spi_id,0 )),
			(uint8_t )(PIOS_SPI_TransferByte(flash_dev->spi_id,0 )));
	PIOS_Flash_Jedec_ReleaseBus();
...
//	printf("   ADC %d %d %d %d %d %d  ", 1,2,3,4,5,6);
	if(PIOS_Flash_Jedec_Validate(flash_dev) != 0)
		return -1;

//	uint8_t ret;
//	uint8_t out[] = {flash_dev->cfg->sector_erase, (addr >> 6) & 0xff, (addr << 2) & 0xff , 0xA5 };
//	uint8_t out[] = {flash_dev->cfg->sector_erase, (addr >> 3) & 0xff, (addr << 5) & 0xff , 0x00 };
	//AT45_page_earse
	...
	//	PIOS_LED_Toggle(PIOS_LED_GREEN);
#if defined(FLASH_FREERTOS)
		vTaskDelay(1);
#endif
	}
...
#endif
		if ((i++) % 10000 == 0)
			PIOS_LED_Toggle(PIOS_LED_ALARM); //PIOS_LED_HEARTBEAT
...
// AT45_bufTo_ROM
	...
	/*
	uint8_t out[4] = {JEDEC_BUFFER1_WRITE, 0x00, (addr >> 8) & 0xff, addr & 0xff};
...
// AT45_buf_ToRom
	uint8_t out1[4] = {JEDEC_BUFFER1_MM_PROGRAM, (addr >> 6) & 0xff, (addr << 2), 0x00 };
...
//	uint8_t out[] = {0x82, (addr >> 6) & 0xff, (addr << 2) & 0xff, 0x00};
	if(PIOS_SPI_TransferBlock(flash_dev->spi_id,out,NULL,sizeof(out),NULL) < 0) {
		PIOS_Flash_Jedec_ReleaseBus();
		return -1;
	}
...
// Keep polling when bus is busy too
	while(!PIOS_Flash_Jedec_Busy()) {
//		PIOS_LED_Toggle(PIOS_LED_YELLOW);
#if defined(FLASH_FREERTOS)
		vTaskDelay(1);
#endif
	}
...
//		printf(" i=%d %d ", (uint8_t )i,(uint8_t )buffOffset);
/*
		printf(" i=%d offs %d %d", (uint8_t )i,
//						(uint8_t )(buffOffset>>24),
//						(uint8_t )(buffOffset>>16),
//						(uint8_t )(buffOffset>>8),
						(uint8_t )buffOffset,
						(uint8_t )chunk->len
				);
*/
...
//	PIOS_Flash_Jedec_PageToBuffer(1, addr);
//	PIOS_Flash_Jedec_BufferRead (1, 0,  data, len);  // red buffer from address 0x00
...
//	PIOS_Flash_Jedec_BufferRead (1, 0,  data, len);  // red buffer from address 0x00
		  ...
		  out1[0] = JEDEC_BUFFER1_READ;  // 0xd4
	  	  out1[1] = 0x00;
		  out1[2] = 0x00; //(unsigned char)(IntBufferAdr >> 8);
		  out1[3] = 0x00; //(unsigned char)(IntBufferAdr );
		  out1[4] = 0x00;
...
// Return buffer offset from selected page
...
//	uint8_t out[] = {0xD2, (addr >> 6) & 0xff, (addr << 2) & 0xff , 0x00, 0x00, 0x00, 0x00, 0x00};
...
int32_t PIOS_Flash_Jedec_PageToBuffer(unsigned char BufferNum, uint16_t PageAdr)
{
  ...
  if (BufferNum==1)
	  out[0] = JEDEC_PAGE_TO_BUFFER_1; // 0x53
  else
